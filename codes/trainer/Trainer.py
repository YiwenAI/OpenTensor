...
class Trainer():
    ...
    def __init__(self,
        ...
        self.device = device
        self.self_play_device = device
        self.entropy_loss = torch.nn.CrossEntropyLoss()
        self.quantile_loss = QuantileLoss(device=self.device)
    ...
    def generate_synthetic_examples(self,
        ...
        illeagal_num = 0
        leagal_num = 0
        for _ in tqdm(range(samples_n*10)):
            R = random.randint(1, R_limit)
            sample = np.zeros((S_size, S_size, S_size), dtype=np.int32)  # (4, 4, 4)
            states = []        
            actions = []
            rewards = []
            previous_temp_matrix = []
            for r in range(1, (R+1)):
                ct = 0
                while True:
                    u = np.random.choice(coefficients, size=(S_size,), p=prob, replace=True)  # (4, )
                    v = np.random.choice(coefficients, size=(S_size,), p=prob, replace=True)  # (4, )
                    w = np.random.choice(coefficients, size=(S_size,), p=prob, replace=True)  # (4, )
                    ct += 1
                ...
                previous_temp_matrix.append(str(list(np.outer(u, v).flatten())))
                previous_temp_matrix.append(str(list(np.outer(u, w).flatten())))
                previous_temp_matrix.append(str(list(np.outer(v, w).flatten())))

                sample = sample + outer(u, v, w)  # (4, 4, 4)
                action = np.stack([u, v, w], axis=0)  # (3, 4)
                actions.append(canonicalize_action(action))
                states.append(sample.copy())
                rewards.append(-r)

            if len(set(previous_temp_matrix)) != len(previous_temp_matrix):
                illeagal_num += 1
            else:
                leagal_num += 1
            ...
            if leagal_num == samples_n:
                break
                
        if save_path is not None:
            np.save(save_path, np.array(total_results, dtype=object))

        print("illeagal num: ", illeagal_num)
        print("leagal num: ", leagal_num, len(total_results))
    ...
    def learn(self,
        ...
        # self.synthetic_examples  list  每一个 [states, actions, rewards]
        # Dataloader.
        ...
        for i in tqdm(range(old_iter, self.iters_n)):
            ...
            try:
                ...
                if i %10==0:
                    dataloader.dataset._permutate_traj(trajs_n=50000)
    ...
    def infer(self,
        ...
        for step in tqdm(range(step_limit)):
            ...
            if terminate_flag:
                print("Steps ", step, " We get to the end!")
                break
...
if __name__ == '__main__':
    ...
    # trainer.infer()
    # trainer.infer(resume="./exp/500000_head_16/1695973137/ckpt/it0600000.pth", no_base_change=True)
    # import pdb; pdb.set_trace()
    # trainer.generate_synthetic_examples(samples_n=500000, save_path="../../data/500000_T5_scalar3.npy")
    trainer.learn(resume=None,
                  example_path="./data/500000_T5_scalar3.npy")
    # trainer.learn(resume=None)
    # trainer.learn(resume=None,
